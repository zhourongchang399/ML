...
def plot_tree(node, parent_name='root', graph=None, pos=None, level=1, feature_names=None):
    if graph is None:
        graph = nx.DiGraph()
    ...
    if node.is_leaf:
        ...
        if node.left is not None:
            ...
            plot_tree(node.left, child_name, graph=graph, pos=pos, level=level + 1, feature_names=feature_names)

        if node.right is not None:
            child_name = f"{parent_name+feature_name}_R"
            graph.add_edge(parent_name, child_name)
            plot_tree(node.right, child_name, graph=graph, pos=pos, level=level + 1, feature_names=feature_names)
...
def visualize_tree(tree_root, feature_names):
    ...
    nx.draw(graph, pos, with_labels=True, font_weight='bold', node_size=3000, node_color='skyblue', font_size=8)
    plt.show()
